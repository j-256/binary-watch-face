...
The main, screenshot, and prototype variants of `res/raw/watchface.xml` are generated by `tools/generate_watchface.py`. Shared visibility budgets and graph geometry live in `design/visibility.json`; the generator also writes History's `GraphVisibility.java`. Edit these authoring sources, regenerate, and commit the sources and generated outputs together. `python3 tools/generate_watchface.py --check` must report no difference before committing. See `docs/visibility-model.md` for contrast reports and rendered checks.
