...
VISIBILITY = visibility.load_model()
...
PROTOTYPE_OUTPUT = PROJECT_ROOT / "watchface/src/prototype/res/raw/watchface.xml"
...
# Solve the face opacity after accounting for the provider's trace alpha
HISTORY_ALPHA_FAINT = VISIBILITY.face_alpha("faint")
HISTORY_ALPHA_SUBTLE = VISIBILITY.face_alpha("subtle")
HISTORY_ALPHA_CLEAR = VISIBILITY.face_alpha("clear")
...
WEIGHT_UNIFORM_ACTIVE_ALPHA = VISIBILITY.weight_alpha("uniform")
WEIGHT_LIT_ALPHA = VISIBILITY.weight_alpha("lit")
WEIGHT_UNLIT_ALPHA = VISIBILITY.weight_alpha("unlit")
...
AMBIENT_APPEARANCE_CHOICES = (
    AmbientAppearanceChoice("dim_color", "ambient_appearance_dim_color", VISIBILITY.ambient_alpha("dim"), True),
    AmbientAppearanceChoice("TRUE", "ambient_appearance_normal_color", VISIBILITY.ambient_alpha("normal"), True),
    AmbientAppearanceChoice("bright_color", "ambient_appearance_bright_color", VISIBILITY.ambient_alpha("bright"), True),
    AmbientAppearanceChoice("dim_mono", "ambient_appearance_dim_mono", VISIBILITY.ambient_alpha("dim"), False),
    AmbientAppearanceChoice("FALSE", "ambient_appearance_normal_mono", VISIBILITY.ambient_alpha("normal"), False),
    AmbientAppearanceChoice("bright_mono", "ambient_appearance_bright_mono", VISIBILITY.ambient_alpha("bright"), False),
)
...
def add_user_configurations(root: ET.Element) -> None:
    ...
    for option_id, slot_ids in ALL_COMPLICATION_LAYOUTS.items():
...
def add_battery(scene: ET.Element) -> None:
    ...
    for bit_count in range(BATTERY_MAX_BITS, 1, -1):
        threshold = 2 ** (bit_count - 1)
        expression = element(binary_expressions, "Expression", name=f"battery_uses_{bit_count}_bits")
        # Disjoint ranges avoid overlapping matches falling through on Wear OS 7
        expression.text = f"[BATTERY_PERCENT] >= {threshold} && [BATTERY_PERCENT] < {threshold * 2}"
    for bit_count in range(BATTERY_MAX_BITS, 1, -1):
...
def add_history_background(scene: ET.Element) -> None:
    x, y, width, height = HISTORY_BOUNDS
    slot = element(
    ...
    add_variant(slot, "alpha", VISIBILITY.ambient_graph_alpha)
    element(slot, "BoundingBox", x=0, y=0, width=width, height=height)
    element(slot, "DefaultProviderPolicy", primaryProvider=HISTORY_PROVIDER,
            primaryProviderType="PHOTO_IMAGE", defaultSystemProvider="EMPTY", defaultSystemProviderType="EMPTY")
...
def build_watchface(*, heart_rate: int | None = None, prototype: bool = False) -> ET.Element:
    VISIBILITY.require_calibration()
    ...
    root.append(ET.Comment(" Visibility budgets: design/visibility.json "))
    ...
    if prototype:
        ...
        for configuration_id, value in overrides.items():
            configuration = root.find(f"./UserConfigurations/*[@id='{configuration_id}']")
            assert configuration is not None
            configuration.set("defaultValue", value)
            flavor_option = root.find(f"./UserConfigurations/Flavors/Flavor[@id='{DEFAULT_FLAVOR_ID}']/Configuration[@id='{configuration_id}']")
            assert flavor_option is not None
            flavor_option.set("optionId", value)
        flavor = root.find(f"./UserConfigurations/Flavors/Flavor[@id='{DEFAULT_FLAVOR_ID}']")
        assert flavor is not None
        history = element(flavor, "ComplicationSlot", slotId=HISTORY_SLOT_ID)
        element(history, "DefaultProviderPolicy", primaryProvider=HISTORY_PROVIDER,
                primaryProviderType="PHOTO_IMAGE", defaultSystemProvider="EMPTY", defaultSystemProviderType="EMPTY")

    scene = element(root, "Scene", backgroundColor=COLOR_BLACK)
    add_background(scene)
    add_history_background(scene)
    add_clock(scene)
    add_tick_ring(scene)
    add_date(scene)
    add_battery(scene)
...
def render_watchface(*, heart_rate: int | None = None, prototype: bool = False) -> str:
    root = build_watchface(heart_rate=heart_rate, prototype=prototype)
...
def parse_arguments(arguments: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate Binary Watch Face WFF definitions and shared History visibility constants",
    ...
    parser.add_argument(
        ...
        "--check",
        action="store_true",
        help="fail when a generated definition or shared visibility file is stale",
    )
    parser.add_argument(
        "-o",
        "--output",
...
def main(arguments: Sequence[str] | None = None) -> int:
    ...
    targets = (
        ((options.output, options.heart_rate, False),)
        if options.output is not None
        else (
            (DEFAULT_OUTPUT, None, False),
            (SCREENSHOT_OUTPUT, SCREENSHOT_HEART_RATE, False),
            (PROTOTYPE_OUTPUT, None, True),
        )
    )

    outputs = {output: render_watchface(heart_rate=heart_rate, prototype=prototype)
               for output, heart_rate, prototype in targets}
    if options.output is None:
        outputs[visibility.JAVA_OUTPUT] = VISIBILITY.render_java()
    ...
    if options.check:
        ...
        for output, generated in outputs.items():
            if not output.exists() or output.read_text(encoding="utf-8") != generated:
                ...
                print(f"Generated file is up to date: {output}")
        for output in stale:
            print(f"Generated file differs from {output}", file=sys.stderr)
