...
Default Binary Pulse to the subtle history layout. Bias the plot toward the lower half of the image to use the gap below the binary rows. Keep its lower edge and image bounds above the system reserve, and retain the side captions in their clock gutters. Give the trace enough contrast and thickness to follow at watch size, its optional time marks slightly more contrast, and the side timestamps more still while remaining subordinate to the clock and native readouts. Keep the fill and range shading faint independently of the trace; dimming the entire plot compounds with the face's brightness selector and can make even Clear unreadable. Faint provides a restrained trace; Subtle balances visibility with the binary clock; Clear provides stronger contrast. Labels stay off by default; tapping the background provides timestamps and precise inspection in the companion app.

Use the [visibility model](visibility-model.md) when tuning this hierarchy. Specify contrast weights and geometry in the shared source, derive the provider and face alphas together, and verify the final composite. Keep literal backdrop-opacity settings independent. The calibration uses a reference palette; inspect the palette report and real rendering before extending its claims to other colors or backgrounds.

Optional time marks are vertical ticks, dots, or upright triangles centered on the heart-history trace, without a conventional axis or grid. Keep ticks vertical and triangles upright as the trace rises and falls. None hides all marks without affecting labels. Preserve gaps and use equal elapsed intervals so a wearer can count inward from the two side times. Control the marks separately from the side labels so the timestamps remain readable at native watch size. Keep the side times in the outer margins beside the hour row, clear of the moving bezel tick, round screen edge, and every provider layout. Only the optional range and status notices use the caption area beneath the date.
