...
2. Set Binary Pulse's **Layout** as instructed. Keep the same ordinary complication count when switching between graph and non-graph layouts. For a graph-on run, verify the graph is visible; its background slot is fixed to the history provider. The app cannot verify the face's layout automatically; the confirmation is your declaration of the setup.
