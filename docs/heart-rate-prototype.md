...
Updating an existing installation preserves its chosen layout. To use the quieter default on that installation, select **Graph: Subtle; slots: Two**, or the subtle option for your preferred provider count. Keep **Graph labels: None** in Binary Heart History to show timestamps only in the app. The decimal backdrop defaults to active mode and retains its own color, opacity, layout, and visibility controls. An installation that already has the backdrop hidden keeps that choice after an update; select **Backdrop visibility: Active only** or **Active and AOD** to show the numbers with the graph.

Wear OS allows only one active setting to control complication-slot enablement. Graph visibility and provider count therefore share the Layout & brightness selector. Choosing a layout without history disables the image slot completely. The fixed **Heart history graph** provider prevents an unrelated image or tap action from replacing history. Ordinary complication slots remain customizable. Tapping an exposed part of the history background opens Binary Heart History; ordinary complications retain their own actions. The graph renders below the ordinary complications, whose opaque backgrounds preserve their contrast and whose normal actions remain reachable.
