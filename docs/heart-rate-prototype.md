...
Wear OS allows only one active setting to control complication-slot enablement. Graph visibility and provider count therefore share the Layout selector. Choosing a layout without history disables the image slot completely. The fixed **Heart history graph** provider prevents an unrelated image or tap action from replacing history. Ordinary complication slots remain customizable. Tapping an exposed part of the history background opens Binary Heart History; ordinary complications retain their own actions. The graph renders below the ordinary complications, whose opaque backgrounds preserve their contrast and whose normal actions remain reachable.
